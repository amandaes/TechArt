...
public static class Perlin
{
    ...
    public static float Noise(float x)
    {
        ...
        var X = Mathf.FloorToInt(x) & 0xff;             //floortoint is biggest int that is smaller than input value
        // x: decimal portion of x as FLOAT
        x -= Mathf.Floor(x);                            //subtracting the X
        // u: 6x^5 - 15x^4 - 10x^3
        ...
        return Lerp(u, Grad(perm[X], x), //a 
            Grad(perm[X + 1], x - 1)) //b, X+1 is checking the next int
            * 2; 
    }
    ...
    {
        return (hash & 1) == 0 ? x : -x;        //bitwise operation like an if statement, doing an even or odd check to return 0 or 1
    }
